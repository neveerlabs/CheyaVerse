  ...
  async function doRename() {
    ...
    try {
      const res = await fetch(
        `/api/media/${renameItem.id}/rename?uid=${encodeURIComponent(uid)}`,
        {
          method: "PATCH",
      ...
      if (res.ok) {
        setLocalItems((prev) =>
          prev.map((x) => (x.id === renameItem.id ? { ...x, filename: name } : x)),
        );
      ...
      if (res.status === 404 || reason === "not_found") {
        ...
        showToast("Gagal rename (cek policy UPDATE)");
      } else {
        showToast(`Gagal mengubah nama (${res.status})`);
      }
      setRenameItem(null);
      closeMenu();
    } catch {
  ...
  return (
    <>
      ...
      {renameItem && (
        <div
          onClick={(e) => {
            if (e.target === e.currentTarget && !busy) setRenameItem(null);
          }}
        ...
        >
          <div className="w-full max-w-[380px] rounded-2xl bg-white border border-line p-6 animate-fade-up">
            <h2 className="text-[16px] font-semibold text-center mb-1 text-ink">Ubah Nama</h2>
            <p className="text-[12.5px] text-ink-soft text-center mb-5">
              Masukkan nama baru untuk file ini.
            </p>
            <input
              value={renameValue}
              ...
              maxLength={200}
              autoFocus
              className="w-full px-3.5 py-3 rounded-xl bg-[#fafafa] border-[1.5px] border-line
                         text-ink text-[14px] outline-none transition-all mb-4
                         focus:border-ink focus:bg-white font-[inherit]"
            />
            <div className="flex gap-2.5">
              ...
              <button
                ...
                className="flex-1 px-4 py-3 rounded-xl bg-ink hover:bg-accent-hover text-white
                           text-[13px] font-semibold active:scale-[.97] transition-all
                           disabled:opacity-60"
              >
